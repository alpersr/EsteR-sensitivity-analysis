  ...
  if (!is.logical(simulation_results[[i]]$distribution)){
    metric_results <- rbind(metric_results,
                            data.frame("parameter1" = simulation_results[[i]]$parameter1,
                                       ...
                                       "hdr80_left" = simulation_results[[i]]$distribution$dates[1] + simulation_results[[i]]$hdr80$qend*60*60*24,
                                       "hdr80_right" = simulation_results[[i]]$distribution$dates[1] + simulation_results[[i]]$hdr80$qstart*60*60*24))
